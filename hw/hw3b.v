...
Proof.
  intros A B C g f injg injf x x' eq.
  rewrite comp_ok in eq.
  rewrite comp_ok in eq.
  apply injg in eq.
  apply injf in eq.
  exact eq.
...
Proof.
  intros A B f inv x x' eq.
  unfold invertible in inv.
  destruct inv as [g [eq' eq'']].
  assert (h : app g (app f x) = app g (app f x')). {
    rewrite eq.
    reflexivity.
  }
  rewrite <- comp_ok in h.
  rewrite <- comp_ok in h.
  rewrite eq'' in h.
  rewrite id_ok in h.
  rewrite id_ok in h.
...
Theorem inv_then_sur :
  forall {A B} (f : Func A B), invertible f -> surjective f.
Proof.
  intros A B f [g [eq eq']] b.
  exists (app g b).
  rewrite <- comp_ok.
  rewrite eq.
  rewrite id_ok.
...
Proof.
  ...
  - split.
    + unfold symmetric.
      intros A B bij.
      destruct bij as [f [injf surf]].
      assert (h : invertible f). {
        apply bij_then_inv.
        - exact injf.
        - exact surf.
      }
      destruct h as [g [eq eq']].
      exists g.
      assert (h' : invertible g). {
        exists f.
        split.
        - exact eq'.
        - exact eq.
      ...
      * apply inv_then_sur.
        exact h'.
    + unfold transitive.
      intros A B C [f [injf surf]] [g [injg surg]].
      exists (comp g f).
      split.
      * apply inj_closed_under_comp; assumption.
      * apply sur_closed_under_comp; assumption.
Qed.
