...
(** * Homework problems
    Instructions:
        ...
        - Deadline: Mar 12 @ 23:55
 *)

(** We would like to prove that the composition of surjective
      functions is surjective. *)
Theorem sur_closed_under_comp :
  forall {A B C} (g : Func B C) (f : Func A B),
    surjective g -> surjective f -> surjective (comp g f).
Proof.
  intros A B C g f surg surf.
  unfold surjective.
  intros y.
  destruct (surg y) as [b eq].
  destruct (surf b) as [a eq'].
  exists a.
  rewrite comp_ok.
  rewrite eq'.
  exact eq.
...
(** The constant function [f(x) = c] for [c ∈ nat] is not injective or
      surjective. For Coq, again we define our function *)
Definition constant {A} (c : A) := by_formula (fun x : A => c).

(** You may find the following lemma helpful. *)
Lemma constant_ok :
  forall {A} (c x : A), app (constant c) x = c.
Proof.
  intros A c x.
  unfold constant.
  rewrite by_formula_ok.
...
(** Prove that for any [c ∈ nat], the constant function at [c] is
      not injective. *)
Theorem const_not_inj :
  forall c : nat, ~ injective (constant c).
Proof.
  intros c inj.
  unfold injective in inj.
  set (h := inj 0 1).
  assert (h' : app (constant c) 0 = app (constant c) 1). {
    rewrite constant_ok.
    rewrite constant_ok.
    reflexivity.
  }
...
(** Prove that for any [c ∈ nat], the constant function at [c] is
      not surjective. *)
Theorem const_not_sur :
  forall c : nat, ~ surjective (constant c).
Proof.
  intros c sur.
  unfold surjective in sur.
  set (h := sur (S c)).
  destruct h as [x eq].
  rewrite constant_ok in eq.
  apply n_Sn in eq.
  exact eq.
Qed.
